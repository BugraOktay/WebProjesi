...
function email_gonder() {
    ...
    app.post('/email', (req, res) => {
        ...
        var sqlQuery = "UPDATE webproje.ogrenci SET sifre = ? WHERE eposta = ?;"
        var sqlQuery1 = "UPDATE webproje.hoca SET sifre = ? WHERE eposta = ?;"
...
//////////////////////////////////////
db.query("SELECT * FROM webproje.ogrenci", (error, results, fields) => {
    if (error) {
        throw (error);
    }
    ...
    app.use("/k-antrenman", function (req, res) {
        ...
        if (req.session.tur == 'h') {
            ...
            const sqlQuery = "SELECT * FROM webproje.atama WHERE hoca=?"
            ...
            const sqlQuery = "SELECT * FROM webproje.hareket WHERE ogrenci=?"

            db.query(sqlQuery, req.session.email, (error, results, fields) => {
                if (error) throw error;
                if(results[0]!=null){
                const sqlQuery1 = "SELECT * FROM webproje.spor WHERE hareket=?"
                db.query(sqlQuery1, results[0].hareket1, (error, results1, fields)=>{
                    if (error) throw error;
                    const sqlQuery2 = "SELECT * FROM webproje.spor WHERE hareket=?"
                db.query(sqlQuery1, results[0].hareket2, (error, results2, fields)=>{
                    if (error) throw error;
                    const sqlQuery3 = "SELECT * FROM webproje.spor WHERE hareket=?"
                db.query(sqlQuery1, results[0].hareket3, (error, results3, fields)=>{
                    if (error) throw error;
                    const sqlQuery4 = "SELECT * FROM webproje.spor WHERE hareket=?"
    ...
    app.use("/k-beslenme", function (req, res) {
        if (req.session.tur == 'h') {
            ...
            const sqlQuery = "SELECT * FROM webproje.beslenme WHERE ogrenci=?"
    ...
    app.use("/k-iletisim", function (req, res) {
        const sqlQuery = 'SELECT * FROM webproje.mesaj WHERE alici = ?;'
        db.query(sqlQuery, [req.session.email], (error, results, fields) => {
            if (error) throw error;
...
app.post('/veri-ekle', (req, res) => {
    ...
    if (oki == 'Danisan') {
        ...
        var sqlQuery2 = 'SELECT * FROM webproje.hoca WHERE hedef=?;';
        db.query(sqlQuery2, hedef, (error, results, fields) => {
            if (error) throw error;
            hoca_mail = results[0].eposta;
            console.log(hoca_mail);
            
            var sqlQuery1 = 'INSERT INTO webproje.atama (ogrenci,hoca) VALUES (?,?)';
        ...
        sqlQuery = 'INSERT INTO webproje.ogrenci (adsoyad, dogum, cinsiyet, eposta, sifre, telefon,hedef) VALUES (?,?,?,?,?,?,?)';
    }
    else if (oki == 'Hoca') {
        sqlQuery = 'INSERT INTO webproje.hoca (adsoyad, dogum, cinsiyet, eposta, sifre, telefon,hedef) VALUES (?,?,?,?,?,?,?)';
    }
    else {
...
app.post('/mesaj_gonder', (req, res) => {
    ...
    const sqlQuery = 'INSERT INTO webproje.mesaj (gonderen,alici,mesaj) VALUES (?,?,?)';
...
app.post('/guncel', (req, res) => {
    ...
    var sqlQuery = "UPDATE webproje.ogrenci SET adsoyad=?,cinsiyet=?,eposta=?,sifre=?,telefon=?,dogum=? WHERE eposta = ?;"
    var sqlQuery1 = "UPDATE webproje.hoca SET adsoyad=?,cinsiyet=?,eposta=?,sifre=?,telefon=?,dogum=? WHERE eposta = ?;"
...
app.post('/beslenme', (req, res) => {
    ...
    var dizi = [renk, alan];
    var sqlQuery = "INSERT INTO webproje.beslenme (ogrenci,pazartesi) VALUES (?,?);"
...
app.post('/spor', (req, res) => {
    ...
    var sqlQuery = "INSERT INTO webproje.hareket (ogrenci,hareket1,hareket2,hareket3,hareket4) VALUES (?,?,?,?,?);"
